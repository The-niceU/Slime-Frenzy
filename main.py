...
font_20 = pygame.font.Font(os.path.join(FONTS_DIR, "IBMPlexSans-Regular.ttf"), 20)
font_15 = pygame.font.Font(os.path.join(FONTS_DIR, "IBMPlexSans-Regular.ttf"), 15)
...
def game_loop(load, mapName, Diff=""):
    ...
    width, height = 16, 12
    pygame.mixer.music.load(os.path.join(AUDIO_DIR, "game.mp3"))  # 游戏专用音乐
    ...
    if load:
        if mapName == "Default":
            file = open(os.path.join(SAVES_DIR, "Classic.txt"), "r")
        elif mapName == "Twisty":
            file = open(os.path.join(SAVES_DIR, "TwistyTowers.txt"), "r")
    ...
    while game_run:
        ...
        if len(Monsters) == 0:
            ...
            if mapName == "Default":
                file = open(os.path.join(SAVES_DIR, "Classic.txt"), "w")
            elif mapName == "Twisty":
                file = open(os.path.join(SAVES_DIR, "TwistyTowers.txt"), "w")
            file.write(data)

            file = open(os.path.join(SAVES_DIR, "saveFile.txt"), "r")
            data = file.readline().split("#")
            if len(data) != 4:
            ...
            file = open(os.path.join(SAVES_DIR, "saveFile.txt"), "w")
